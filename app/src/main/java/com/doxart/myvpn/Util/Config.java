package com.doxart.myvpn.Util;

public class Config {
    public static String REVENUE_CAT_APP_ID = "REPLACE_REVENUE_CAT_APP_ID";
    public static String IRONSOURCE_AD_ID = "REPLACE_IRONSOURCE_AD_ID";

    public static String INTERSTITIAL_AD_ID = "REPLACE_INTERSTITIAL_AD_ID";

    public static String NATIVE_AD_ID = "REPLACE_NATIVE_AD_ID";

    public static String REWARDED_AD_ID = "REPLACE_REWARDED_AD_ID";

    public static String BANNER_AD_ID = "REPLACE_BANNER_AD_ID";
}
